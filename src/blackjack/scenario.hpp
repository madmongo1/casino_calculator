...
inline auto operator<<(std::ostream &os, player_action pa) -> std::ostream & {
  switch (pa) {
  case player_action::double_down:
    os << "double down";
    break;
  case player_action::hit:
    os << "hit";
    break;
  case player_action::stick:
    os << "stick";
    break;
...
struct scenario_result : outcome {
  scenario_result(player_action action) : action(action), outcome() {}
...
struct scenario {
  ...
  static scenario_result best_of(result_vector const &v) {
    assert(!v.empty());
    auto first = v.begin();
    auto last = v.end();
  ...
  auto hit_player(shoe const &s, player_hand const &p, dealer_hand const &d)
      -> outcome {
...
*/
  ...
  auto dealers_turn(shoe const &s, score const &player_score,
